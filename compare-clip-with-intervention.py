...
INTERV_CSV = "results_csv_interv/plop_per_block.csv"
OUT_CSV = "results_csv_interv/plop_per_block_comparison.csv"
...
# Merge
df = pd.merge(base, interv, on=merge_cols, suffixes=('_orig', '_interv'))

# Compute absolute and percent differences for each metric
for metric in ['actual', 'random', 'nfn']:
    df[f'{metric}_delta'] = df[f'{metric}_interv'] - df[f'{metric}_orig']
...
def summarize_changes(df, metric='nfn', topk=10):
    ...
    print(biggest[[*merge_cols, f'{metric}_orig', f'{metric}_interv', f'{metric}_delta', f'{metric}_pct']])
...
for branch in df['branch'].unique():
    ...
    for metric in ['nfn', 'actual']:
        ...
        plt.tight_layout()
        plt.savefig(f"results_csv_interv/delta_{metric}_{branch}.png")
        plt.close()
print("Saved separate delta plots for vision and text branches to results_csv_interv/.")


print("Saved change plots to results_csv_interv/.")
...
if not drop_blocks.empty:
    ...
    print(drop_blocks[[*merge_cols, 'nfn_orig', 'nfn_interv', 'nfn_delta', 'nfn_pct']])
else:
