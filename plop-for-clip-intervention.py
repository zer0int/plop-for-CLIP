...
os.makedirs("results_csv_interv", exist_ok=True)
os.makedirs("results_plots_interv", exist_ok=True)
...
csv_path = os.path.join("results_csv_interv", "plop_per_block.csv")
...
for branch, groups in [("vision", vision_types), ("text", text_types)]:
    ...
    # CSV export
    csv_branch = os.path.join("results_csv_interv", f"plop_summary_{branch}.csv")
...
# 3. Plot overview
def plot_group(group_result, branch, save_dir="results_plots_interv"):
...
def plot_per_block(per_block, branch, save_dir="results_plots_interv"):
